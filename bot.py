...
Files:
    bayanaa_style.json.gz  (losslessly compressed style model)
...
MODEL_FILE = Path(os.getenv("STYLE_MODEL", "bayanaa_style.json.gz"))
...
def load_style_model(path):
    """Load either the original JSON model or the losslessly compressed .gz model."""
    if path.suffix.lower() == ".gz":
        with gzip.open(path, "rt", encoding="utf-8") as f:
            return json.load(f)
    return json.loads(path.read_text(encoding="utf-8"))

MODEL = load_style_model(MODEL_FILE)
BOOK = MODEL.get("book", {})
...
def style_score(board, move):
    ...
    total_moves = max(1, MODEL.get("moves_seen", 1))
